...
$sql = "SELECT date, (select ss_programs.program_name from  ss_programs where ss_programs.id_program = ss_preregistration.id_program) AS nombre_programa, paternal_last_name, maternal_last_name, first_name, state, home_phone, cell_phone, email FROM `ss_preregistration` WHERE date >= '2012-01-01' AND date <= '2012-06-30' ORDER BY date ASC";
