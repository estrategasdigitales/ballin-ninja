...
<p>SI T&Uacute; SABES, T&Uacute; GANAS<br />&iexcl;Mucho &eacute;xito!</p>
		...
		mail($_POST['email1'],'Confirmación Trivia Ibero',$message_registro,$headers);
		...
		for($i = 1; $i <= $_POST['cont']; $i++){
			...
			if($_POST['correo_'.$i] != NULL){
				...
				<p>S&iacute;guenos en <a href="http://www.facebook.com/diplomados.uia/app_203351739677351">Facebook</a> para conocer m&aacute;s detalles.</p>
				 
<p>SI T&Uacute; SABES, T&Uacute; GANAS</p>
				 
				<p>ATTE:<br /> 
				Direcci&oacute;n de Educaci&oacute;n Continua, Universidad Iberoamericana Ciudad de M&eacute;xico.</p>
		<p style="font-size:11px;">*No aplica en diplomados, programas de licenciatura o posgrado. V&aacute;lido en la Ibero Ciudad de M&eacute;xico y sedes externas.</p>
		<p><img src="http://dec-uia.com/trivia_2012/images/banner_mail.jpg" /></p>';
		...
		//session_start();
		//$_SESSION['mm_id'] = $row_usuarios1['id_participante'];
		//$_SESSION['mm_username'] = $row_usuarios1['email'];
		//$_SESSION['mm_user'] = $row_usuarios1['nombre'].' '.$row_usuarios1['apellido'];
		//header('Location: http://www.dec-uia.com/trivia_2012/preguntas.php');
		//header("Location: http://www.dec-uia.com/trivia_2012/registro_b.php");
		header("Location: extras.php?type=0&register=true&reg_news=".$_POST['reg_news']."&email=".$_POST['email1']);
	
	}else{
	
		header("Location: extras.php?type=0&user=true&reg_news=".$_POST['reg_news']."&email=".$_POST['email1']);
