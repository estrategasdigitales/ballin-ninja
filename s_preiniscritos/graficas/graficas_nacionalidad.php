...
if(isset($_POST['fecha_ini']) && $_POST['fecha_ini'] != ''){
	...
	if($_POST['id_discipline'] == 0 && $_POST['id_program'] == 0){
		...
		$query_graficas .= ") ORDER BY nacionalidad ASC";
	...
	switch($query){
		case 1:
			$graph_title = 'Gráfica por nacionalidad<br /><strong>Todas las áreas</strong><br />Del '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'; (TOTAL: '.mysql_num_rows($graficas).')';
		break;
		case 2:
			$graph_title = 'Gráfica por nacionalidad<br /><strong>Área: '.$row_graficas['disciplina'].'</strong><br />Del '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'; (TOTAL: '.mysql_num_rows($graficas).')';
		break;
		case 3:
			$graph_title = 'Gráfica por nacionalidad<br /><strong>'.$row_graficas['program_type'].': '.$row_graficas['programa'].'</strong><br />Del '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'; (TOTAL: '.mysql_num_rows($graficas).')';
		break;
		case 4:
			$graph_title = 'Gráfica por nacionalidad<br /><strong>Área: '.$row_graficas['disciplina'].' | '.$row_graficas['program_type'].': '.$row_graficas['programa'].'</strong><br />Del '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'; (TOTAL: '.mysql_num_rows($graficas).')';
		break;
	}
		
	$cont = 0; 
	$nac_ant = $row_graficas['nacionalidad'];
	$valores = '';
	//$row_nac = mysql_fetch_assoc($nac);
	do{
		if($row_graficas['nacionalidad']==$nac_ant){
			$cont++;
		}else{
			if($nac_ant == ''){$nac_ant = 'Sin especificar';}
			$valores .= "['".utf8_encode($nac_ant)."',".$cont."],";
			$etiquetas .= "'".$cont."',";
			$cont=1;
		}
		$nac_ant=$row_graficas['nacionalidad'];
...
<title>Sistema de seguimiento - Gráfica por nacionalidad</title>
<!-- InstanceEndEditable -->
...
<div id="container">
 ...
 <div class="sombra"> </div>
  ...
  <div class="espacio"> </div>
  <div>
    <form action="graficas_nacionalidad.php" method="post" name="graph_selector" id="graph_selector" onsubmit="return check_graph_form();">
