...
if(isset($_POST['fecha_ini']) && $_POST['fecha_ini'] != ''){
	...
	if($_POST['id_discipline'] == 0 && $_POST['id_program'] == 0){
		...
		$query_graficas .= ") ORDER BY como_se_entero ASC";
	...
	switch($query){
		case 1:
			$graph_title = 'Gráfica por ¿cómo se enteró?<br /><strong>Todas las áreas</strong><br />Del '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'; (TOTAL: '.mysql_num_rows($graficas).')';
		break;
		case 2:
			$graph_title = 'Gráfica por ¿cómo se enteró?<br /><strong>Área: '.$row_graficas['disciplina'].'</strong><br />Del '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'; (TOTAL: '.mysql_num_rows($graficas).')';
		break;
		case 3:
			$graph_title = 'Gráfica por ¿cómo se enteró?<br /><strong>'.$row_graficas['program_type'].': '.$row_graficas['programa'].'</strong><br />Del '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'; (TOTAL: '.mysql_num_rows($graficas).')';
		break;
		case 4:
			$graph_title = 'Gráfica por ¿cómo se enteró?<br /><strong>Área: '.$row_graficas['disciplina'].' | '.$row_graficas['program_type'].': '.$row_graficas['programa'].'</strong><br />Del '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'; (TOTAL: '.mysql_num_rows($graficas).')';
		break;
	}
	...
	$pattern3 = "Periódico";
	$pattern4 = "Revista";
	$pattern5 = "Recomendación";
	$pattern6 = "Email";*/

	$cont = 0; 
	$como_ant = $row_graficas['como_se_entero'];
	$valores = '';
	//$row_nac = mysql_fetch_assoc($nac);
	do{
		if($row_graficas['como_se_entero']==$como_ant){
			$cont++;
		}else{
			if($como_ant == ''){$como_ant = 'Sin especificar';}
			$valores .= "['".utf8_encode($como_ant)."',".$cont."],";
			$etiquetas .= "'".$cont."',";
			$cont=1;
		}
		$como_ant=$row_graficas['como_se_entero'];
...
<title>Sistema de seguimiento - Gráfica por ¿cómo se enteró?</title>
<!-- InstanceEndEditable -->
...
<div id="container">
 ...
 <div class="sombra"> </div>
  ...
  <div class="espacio"> </div>
  <div>
    <form action="graficas_como_se_entero.php" method="post" name="graph_selector" id="graph_selector" onsubmit="return check_graph_form();">
