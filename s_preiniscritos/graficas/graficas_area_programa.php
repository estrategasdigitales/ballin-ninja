...
if(isset($_POST['fecha_ini']) && $_POST['fecha_ini'] != ''){
	//query para obtener las areas a las que puede acceder el usuario logeado
	//mysql_select_db($database_des_preinscritos, $des_preinscritos);
	if($_POST['id_discipline'] == 0){
		
		$query = 1;
		
		$query_graficas = "
		SELECT id_discipline,
		(SELECT discipline FROM disciplines WHERE sp_preinscritos.id_discipline = disciplines.id_discipline) AS disciplina
		...
		$query_graficas .= ") ORDER BY id_discipline ASC";
		
		//echo 'query 1: '. $query_graficas;
				
	}else if($_POST['id_discipline'] != 0){
		
		$query = 2;
		
		$query_graficas = "
		SELECT id_program,
		(SELECT program_name FROM site_programs WHERE sp_preinscritos.id_program = site_programs.id_program) AS programa,
	...
	switch($query){
		case 1:
			$graph_title = 'Gráfica por área/programa<br /><strong>Todas mis áreas</strong><br />Del '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'; (TOTAL: '.mysql_num_rows($graficas).')';
		break;
		case 2:
			$graph_title = 'Gráfica por área/programa<br /><strong>Área: '.utf8_encode($row_graficas['disciplina']).'</strong><br />Del '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'; (TOTAL: '.mysql_num_rows($graficas).')';
		break;
	}
	...
	if($query == 1){
		$cont = 0; 
		$id_prog_ant = $row_graficas['id_discipline'];
		$prog_ant = $row_graficas['disciplina'];
		$valores = '';
		//$row_nac = mysql_fetch_assoc($nac);
		do{
			if($row_graficas['id_discipline']==$id_prog_ant){
				$cont++;
			}else{
				$valores .= "['".utf8_encode($prog_ant)."',".$cont."],";
				$etiquetas .= "'".$cont."',";
				$cont=1;
			}
			$id_prog_ant=$row_graficas['id_discipline'];
		...
		$cont = 0; 
		$id_prog_ant = $row_graficas['id_program'];
		$prog_ant = $row_graficas['programa'];
		$valores = '';
		//$row_nac = mysql_fetch_assoc($nac);
		do{
			if($row_graficas['id_program']==$id_prog_ant){
				$cont++;
			}else{
				$valores .= "['".utf8_encode($prog_ant)."',".$cont."],";
				$etiquetas .= "'".$cont."',";
...
<title>Sistema de seguimiento - Gráfica por área/programa</title>
<!-- InstanceEndEditable -->
...
<div id="container">
 ...
 <div class="sombra"> </div>
  ...
  <div class="espacio"> </div>
  <div>
    <form action="graficas_area_programa.php" method="post" name="graph_selector" id="graph_selector" onsubmit="return check_graph_form();">
