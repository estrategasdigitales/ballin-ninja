...
if(isset($_POST['fecha_ini']) && $_POST['fecha_ini'] != ''){
	...
	if($_POST['id_discipline'] == 0 && $_POST['id_program'] == 0){
		...
		$query_graficas .= ") ORDER BY exalumno DESC";
	...
	switch($query){
		case 1:
			$graph_title = 'Gráfica por exalumno<br /><strong>Todas las áreas</strong><br />Del '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'; (TOTAL: '.mysql_num_rows($graficas).')';
		break;
		case 2:
			$graph_title = 'Gráfica por exalumno<br /><strong>Área: '.$row_graficas['disciplina'].'</strong><br />Del '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'; (TOTAL: '.mysql_num_rows($graficas).')';
		break;
		case 3:
			$graph_title = 'Gráfica por exalumno<br /><strong>'.$row_graficas['program_type'].': '.$row_graficas['programa'].'</strong><br />Del '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'; (TOTAL: '.mysql_num_rows($graficas).')';
		break;
		case 4:
			$graph_title = 'Gráfica por exalumno<br /><strong>Área: '.$row_graficas['disciplina'].' | '.$row_graficas['program_type'].': '.$row_graficas['programa'].'</strong><br />Del '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'; (TOTAL: '.mysql_num_rows($graficas).')';
		break;
	}
		
	$cont = 0; 
	$exalumno_ant = $row_graficas['exalumno'];
	$valores = '';
	//$row_nac = mysql_fetch_assoc($nac);
	do{
		if($row_graficas['exalumno']==$exalumno_ant){
			$cont++;
		}else{
			$valores .= "['".utf8_encode($exalumno_ant)."',".$cont."],";
			$etiquetas .= "'".$cont."',";
			$cont=1;
		}
		$exalumno_ant=$row_graficas['exalumno'];
	...
	if(utf8_encode($exalumno_ant) == 'Si'){
		$val_labels = 'Si';
	}else if(utf8_encode($exalumno_ant) == 'No'){
		$val_labels = 'No';
	}if(utf8_encode($exalumno_ant) == ''){
		$val_labels = 'Sin especificar';
	}
	
	$valores .= "['".$val_labels."',".$cont."]";
	$etiquetas .= "'".$cont."'";
}
?>
...
<title>Sistema de seguimiento - Gráfica si es exalumno</title>
<!-- InstanceEndEditable -->
...
<div id="container">
 ...
 <div class="sombra"> </div>
  ...
  <div class="espacio"> </div>
  <div>
    <form action="graficas_exalumno.php" method="post" name="graph_selector" id="graph_selector" onsubmit="return check_graph_form();">
