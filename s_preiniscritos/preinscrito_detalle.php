...
if(isset($_POST['update_form']) && $_POST['update_form'] == 1){
	...
	if($_POST['td_comment_primercontacto'] != NULL){
		...
		if($totalRows_comment_1 == 0){
			$insert_comment_1 = "INSERT INTO sp_comentarios (id_preinscrito, id_paso, comentario, fecha) VALUES ('".$_POST['id_preinscrito']."','1','".trim($_POST['td_comment_primercontacto'])."','".date('Y-m-d')."')";
			$Result1 = mysql_query($insert_comment_1, $des_preinscritos) or die(mysql_error());
		}else{
			$update_comment_1 = "UPDATE sp_comentarios SET comentario = '".trim($_POST['td_comment_primercontacto'])."' WHERE id_preinscrito = '".$_POST['id_preinscrito']."' AND id_paso = 1";
	...
	if($_POST['td_comment_documentos'] != NULL){
		...
		if($totalRows_comment_1 == 0){
			$insert_comment_1 = "INSERT INTO sp_comentarios (id_preinscrito, id_paso, comentario, fecha) VALUES ('".$_POST['id_preinscrito']."','2','".trim($_POST['td_comment_documentos'])."','".date('Y-m-d')."')";
			$Result1 = mysql_query($insert_comment_1, $des_preinscritos) or die(mysql_error());
		}else{
			$update_comment_1 = "UPDATE sp_comentarios SET comentario = '".trim($_POST['td_comment_documentos'])."' WHERE id_preinscrito = '".$_POST['id_preinscrito']."' AND id_paso = 2";
			$Result1 = mysql_query($update_comment_1, $des_preinscritos) or die(mysql_error());
		}
	...
	if($_POST['td_comment_decse'] != NULL){
		...
		if($totalRows_comment_1 == 0){
			$insert_comment_1 = "INSERT INTO sp_comentarios (id_preinscrito, id_paso, comentario, fecha) VALUES ('".$_POST['id_preinscrito']."','3','".trim($_POST['td_comment_decse'])."','".date('Y-m-d')."')";
			$Result1 = mysql_query($insert_comment_1, $des_preinscritos) or die(mysql_error());
		}else{
			$update_comment_1 = "UPDATE sp_comentarios SET comentario = '".trim($_POST['td_comment_decse'])."' WHERE id_preinscrito = '".$_POST['id_preinscrito']."' AND id_paso = 3";
			$Result1 = mysql_query($update_comment_1, $des_preinscritos) or die(mysql_error());
		}
	...
	if($_POST['td_comment_envioclaves'] != NULL){
		...
		if($totalRows_comment_1 == 0){
			$insert_comment_1 = "INSERT INTO sp_comentarios (id_preinscrito, id_paso, comentario, fecha) VALUES ('".$_POST['id_preinscrito']."','4','".trim($_POST['td_comment_envioclaves'])."','".date('Y-m-d')."')";
			$Result1 = mysql_query($insert_comment_1, $des_preinscritos) or die(mysql_error());
		}else{
			$update_comment_1 = "UPDATE sp_comentarios SET comentario = '".trim($_POST['td_comment_envioclaves'])."' WHERE id_preinscrito = '".$_POST['id_preinscrito']."' AND id_paso = 4";
			$Result1 = mysql_query($update_comment_1, $des_preinscritos) or die(mysql_error());
		}
	...
	if($_POST['td_comment_pagorealizado'] != NULL){
		...
		$query_comment_1 = "SELECT * FROM sp_comentarios WHERE id_preinscrito = ".$_GET['id_preinscrito']." AND id_paso = 5";
		$comment_1 = mysql_query($query_comment_1, $des_preinscritos) or die(mysql_error());
		//$row_comment_1 = mysql_fetch_assoc($comment_1);
		$totalRows_comment_1 = mysql_num_rows($comment_1);
		
		if($totalRows_comment_1 == 0){
			$insert_comment_1 = "INSERT INTO sp_comentarios (id_preinscrito, id_paso, comentario, fecha) VALUES ('".$_POST['id_preinscrito']."','5','".trim($_POST['td_comment_pagorealizado'])."','".date('Y-m-d')."')";
			$Result1 = mysql_query($insert_comment_1, $des_preinscritos) or die(mysql_error());
		}else{
			$update_comment_1 = "UPDATE sp_comentarios SET comentario = '".trim($_POST['td_comment_pagorealizado'])."' WHERE id_preinscrito = '".$_POST['id_preinscrito']."' AND id_paso = 5";
			$Result1 = mysql_query($update_comment_1, $des_preinscritos) or die(mysql_error());
		}
...
<form onsubmit="alert('La información ha sido guardada.');" method="post" action="preinscrito_detalle.php?id_preinscrito=<? echo $_GET['id_preinscrito']; ?>" name="form1" id="form1" enctype="multipart/form-data" <? if($row_paso['envio_decse'] == 0){echo 'onsubmit="return check_decse();"';} ?>>
  ...
  <table width="822" border="0" cellpadding="0" cellspacing="0">
    <tr valign="top">
      <td width="400"><h2>Datos del interesado</h2>
        <p><strong>Nombre completo: </strong><?php echo ucfirst(strtolower($row_preinscrito['a_paterno'])).' '.ucfirst(strtolower($row_preinscrito['a_materno'])).', '.ucwords(strtolower($row_preinscrito['nombre'])); ?></p>
        <p><strong>Fecha de registro: </strong><? echo utf8_encode(strftime("%d de %B de %Y", strtotime($row_preinscrito['fecha_registro']))); ?></p>
        <p><strong>Calle y n&uacute;mero: </strong><?php echo ucfirst(strtolower($row_preinscrito['calle_numero'])); ?> </p>
		<p><strong>Colonia: </strong><?php echo ucwords(strtolower($row_preinscrito['colonia'])); ?></p>
		<p><strong>Del o mpo: </strong><?php echo ucwords(strtolower($row_preinscrito['del_mpo'])); ?></p>
        ...
        <p><strong>RFC: </strong><?php echo utf8_encode(strtoupper($row_preinscrito['rfc'])); ?> </p>
        <p><strong>Correo electr&oacute;nico: </strong><a href="mailto:<?php echo utf8_encode(strtolower($row_preinscrito['correo'])); ?>"><?php echo utf8_encode(strtolower($row_preinscrito['correo'])); ?></a> </p>
        ...
        <p><strong>¿Porqué la Ibero?:</strong> <?php echo ucfirst(strtolower($row_preinscrito['porque_la_ibero'])); ?> </p>
        <p>&nbsp;</p>
          ...
          <tr>
			...
			?>
              ...
              <table width="100%" border="0" cellpadding="0" cellspacing="0" id="table_file">
                ...
                <tr>
                  <td><input name="doc_0" type="file" id="doc_0" value="" /></td>
      ...
      <td><h2>Estatus del proceso</h2>
        <table width="100%" border="0" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <td width="14">&nbsp;</td>
            <td width="201"><p class="anadir_comentario"><a style="cursor:pointer;" onclick="add_small_comment('td_comment_primercontacto','<? echo $comment_1; ?>');"><img src="imagenes/comentario.png" /> a&ntilde;adir - editar comentario</a></p></td>
          </tr>
          <tr>
      ...
      <td><h2>Clasificar al aspirante</h2>
        <table width="100%" border="0" cellpadding="5" cellspacing="0">
          ...
          <tr>
          	...
          	<td width="186" align="left">Atendido</td>
          	</tr>
        </table>
        <h2>&nbsp;</h2>
        <h2>Comentarios</h2>
    ...
    <tr>
      <td colspan="5"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="50%" align="center"><input type="button" name="editar" id="editar" value="Editar información" <? if($row_paso['envio_decse'] == 1 && $row_paso['envio_claves'] == 0){echo 'disabled="disabled"';} ?> onclick="javascrip:window.location='preinscrito_editar.php?id_preinscrito=<? echo $_GET['id_preinscrito']; ?>&id_program=<? echo $row_preinscrito['id_program']; ?>'" /></td>
  ...
  <input type="hidden" name="preinscrito_name" value="<?php echo $row_preinscrito['a_paterno'].' '.$row_preinscrito['a_materno'].', '.$row_preinscrito['nombre']?>" />
