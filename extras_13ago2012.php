...
if(isset($_POST['send']) && $_POST['send'] == true){
	...
	$query = 'INSERT INTO cat2012_empresas (nombre,apellido,empresa,puesto,numero_de_empleados,direccion,email,telefono,dese_ser_contactado) VALUES ("'.$_POST['nombre'].'","'.$_POST['apellido'].'","'.$_POST['empresa'].'","'.$_POST['puesto'].'","'.$_POST['numero_de_empleados'].'","'.$_POST['direccion'].'","'.$_POST['email'].'","'.$_POST['telefono'].'","'.$_POST['deseo_ser_contactado'].'")';
	
	  
	mysql_select_db($database_otono2011, $otono2011);

	$Result1 = mysql_query($query, $otono2011) or die(mysql_error());
...
<script>
function validar(){
	//alert($('input#nombre]').val());
	var cont = 0;
	var msg = 'Favor de proporcionar los siguientes campos:\n';
	if($('input#nombre').val() == ''){
		cont++;
		msg = '*Nombre\n';
	}
	...
	if($('input#numero_de_empleados').val() == ''){
		cont++;
		msg = '*Número de empleados\n';
	}
	...
	if($('input#acepto_terminos').val()==''){
		cont++;
		msg += 'Aceptar el aviso de privacidad';
	}
	if(cont > 0){
		return false;
	}else{
...
<div id="container">
  ...
  <div id= "contenedor_irregular_index" >
    <div id= "type4" class="cuadro_articulos_secciones">
	  ...
	  <img src="imagenes/titles/mailing_formulario.jpg" width="775" height="250" />
    ...
    <div id= "type4" class="rectangulo_abajo_secciones">
      <div class="textos"><!-- InstanceBeginEditable name="contenido" -->
		  	<table width="75%" border="0" align="left" cellpadding="10" cellspacing="0">
		  		<tr>
		  			<td width="100%" valign="top"><h1 style="color:#666;">Reg&iacute;stra a tu empresa y recibe el cat&aacute;logo de diplomados y cursos de oto&ntilde;o 2012</h1></td>
	  			</tr>
		  		<tr>
		  			<td valign="top">
					<? if($_POST['send'] == true){ 
							echo 'Tu registro fue exitoso, a la brevedad te enviaremos el catálogo. Gracias.';
						}else{
							echo 'Llena los siguientes campos y te enviaremos la informaci&oacute;n v&iacute;a mensajer&iacute;a*';
						}
					?></td>
	  			</tr>
		  		<tr>
		  			<td valign="top">
						...
						<form name="registro" action="extras.php" method="post" onsubmit="return validar();">
							<table width="100%" border="0" cellspacing="0" cellpadding="0">
								<tr>
									<td height="35"><label for="nombre"></label>
										Nombre:<br />
										<input type="text" name="nombre" id="nombre" class="input" /></td>
								</tr>
								<tr>
									<td height="35"><label for="apellido"></label>
										Apellido:<br />
										<input type="text" name="apellido" id="apellido" class="input" /></td>
								</tr>
								<tr>
									<td height="35"><label for="empresa"></label>
										Empresa:<br />
										<input type="text" name="empresa" id="empresa" class="input" /></td>
								</tr>
								<tr>
									<td height="35"><label for="puesto"></label>
										Puesto:<br /><input type="text" name="puesto" id="puesto" class="input" /></td>
								</tr>
								<tr>
									<td height="35"><label for="numero_de_empleados"></label>
										N&uacute;mero de empleados:<br /><input type="text" name="numero_de_empleados" id="numero_de_empleados" class="input" /></td>
								</tr>
								<tr>
									<td height="35"><label for="direccion"></label>
										Direcci&oacute;n:<br /><input type="text" name="direccion" id="direccion" class="input" /></td>
								</tr>
								<tr>
									<td height="35"><label for="email"></label>
										E-mail:<br /><input type="text" name="email" id="email" class="input" /></td>
								</tr>
								<tr>
									<td height="35"><label for="telefono"></label>
										Tel&eacute;fono:<br /><input type="text" name="telefono" id="telefono" class="input" /></td>
								</tr>
								<tr>
									<td height="35"><input name="deseo_ser_contactado" type="checkbox" id="deseo_ser_contactado" value="1" />
										Deseo ser contactado por la Direcci&oacute;n de Educaci&oacute;n Continua</td>
								</tr>
								<tr>
									<td height="35"><input type="submit" name="enviar" id="enviar" value="Enviar" class="input" style="width:100px; height:20px; color:#000; background:#CCC; cursor:pointer;" /></td>
								</tr>
								<tr>
									<td height="35" align="center"><a href="politicas_privacidad.php" target="_blank" style="color:#ff0000;"><strong>
										<input name="acepto_terminos" type="checkbox" id="acepto_terminos" value="1" />
										<label for="acepto_terminos"></label>
										He leido y acepto el Aviso de Privacidad</strong></a></td>
								</tr>
								<tr>
									<td height="35" align="left"><p style="font-size:10px;">* Entrega no tiene costo.</p>
										<p style="font-size:10px;">* Env&iacute;o sujeto a disponibilidad de ejemplares.</p>
										<p style="font-size:10px;">* V&aacute;lido &uacute;nicamente en Santa Fe, poniente del Distrito Federal y zonas aleda&ntilde;as.</p></td>
								</tr>
							</table>
