    ...
    <div id= "type4" class="rectangulo_abajo_secciones">
      <div class="textos"><!-- InstanceBeginEditable name="contenido" -->
		  <?php
				  ...
				  /******************************************************************************/
				  /**** SAVE IMAGE UPLOADED TO CORRESPONDING DIRECTORY ****/
					...
					$IMAGE_FILE_NAME = $_POST['email']; //Reemplazo el nombre de la imagen por el email, ya que es muy probable
					                                    //que alguien de la comunidad UIA suba su foto con su nombre y si existen 2 con el mismo nombre pueden reemplazarse.
														
					//$IMAGE_FILE_NAME = $_FILES['foto']['name'];
					//$IMAGE_FILE_SIZE = $_FILES['foto']['size'];
					$IMAGE_FILE_TYPE = $_FILES['foto']['type'];
				
					if($IMAGE_FILE != NULL){
								
						$photosDir = "comunidadUIA/fotos";
						
						// change spaces to underscores in filename
						$imgfile_name = str_replace(" ", "_", $IMAGE_FILE_NAME);
						//$imgfile_name = str_replace("@", "_", $IMAGE_FILE_NAME);
						//$imgfile_name = str_replace(".", "_", $IMAGE_FILE_NAME);
						...
						switch($IMAGE_FILE_TYPE){
							...
							case "image/jpeg":
								$imgfile_name = $imgfile_name . "." . "jpeg";	
								break;
								
							case "image/jpg":
								$imgfile_name = $imgfile_name . "." . "jpg";	
								break;
								
							case "image/gif":
								$imgfile_name = $imgfile_name . "." . "gif";	
								break;
							
							case "image/png":
								$imgfile_name = $imgfile_name . "." . "png";	
								break;							
						}						
						...
						if (($photo->getWidth() > 800) || ($photo->getHeight() > 600)) 
						{s
							if($photo->getWidth() > 800){
								
								$photo->resizeToWidth(800);
								
							}else if($photo->getHeight() > 600){
								
								$photo->resizeToHeight(600);
							}
						}
						*/
						
						// save the photo into the corresponding directory.
						$photo->save($photosDir . "/" . $imgfile_name);
						
					}
					/******************************************************************************/					 				  
				  							
					$mensaje="<b>Nombre:</b> $nombre<br /><br />";
					$mensaje.="<b>Email:</b> $email<br /><br />";
					$mensaje.="<b>Tel&eacute;fono:</b> $telefono<br /><br />";
					$mensaje.="<b>Perfil:</b> $perfil<br /><br />";
					$mensaje.="<b>Foto:</b> http://www.dec-uia.com/site/comunidadUIA/fotos/$imgfile_name<br /><br />";
					$mensaje.="<b>comentario:</b><br /><br />$comentario<br />";
					...
					$mensaje = str_replace("&aacute;", "&aacute;", $mensaje);
					$mensaje = str_replace("&eacute;", "&eacute;", $mensaje);
					$mensaje = str_replace("&iacute;", "&iacute;", $mensaje);
					$mensaje = str_replace("&oacute;", "&oacute;", $mensaje);
					$mensaje = str_replace("&uacute;", "&uacute;", $mensaje);
					$mensaje = str_replace("&ntilde;", "&ntilde;", $mensaje);
					$mensaje = str_replace("&Ntilde;", "&Ntilde;", $mensaje);

					//echo "DESPUES: " . $mensaje;
										
					mail("erika.medina@uia.mx, joboraz@hotmail.com, joboraz@gmail.com", "DEC - La Comunidad Ibero Opina", $mensaje, $headers);
					//mail("jlaa2774@hotmail.com", "DEC - La Comunidad Ibero Opina", $mensaje, $headers);
					
					//para mandar el mail en texto plano.
					//mail("jlaa2774@hotmail.com", "DEC - Forma de Contacto", $mensaje, "FROM:$email");
                ...
                <table width="94%" border="0" align="center" cellpadding="4" cellspacing="0">
                  ...
                  <tr>
                    <td align="center"><p>Gracias por participar
                      en &quot;La comunidad Ibero Opina&quot;.</p></td>
                  </tr>
                  <tr>
                    <td>&nbsp;</td>
                  </tr>
                  <tr>
                    <td align="center"><p><a href="index.php" class="contenido_diploBold">Ir
                      a Inicio</a></p></td>
                  </tr>
                  <tr>
                    <td>&nbsp;</td>
