...
$sql = mysql_query('SELECT * FROM propuestas_programas ORDER BY id_propuesta', $otono2011);
...
<table  border="0" cellpadding="5" cellspacing="0" class="tablas">
	...
	<?php $cont = 0; do { $cont++; ?>
		<tr>
			<td><?php echo $row_sql['propuesta']; ?></a></td>
			<td><?php echo $row_sql['area']; ?></td>
			<td><?php echo $row_sql['nombre']; ?></td>
			<td><?php echo $row_sql['apellido']; ?></td>
			<td><?php echo $row_sql['correo']; ?></td>
			<td><?php echo $row_sql['telefono']; ?></td>
			<!--td><a href="programas_editar.php?id_program=<?php echo $row_sql['id_program']; ?>">Editar</a></td>
			<td><a onclick="eliminar_prog(<?php echo $row_sql['id_program']; ?>);" href="#">Eliminar</a></td-->
		</tr>
