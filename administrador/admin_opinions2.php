...
<div id="container">
  ...
  <div id="contenedor_irregular_index" style="width:800px;"><!-- InstanceBeginEditable name="contenido" -->
        ...
        <tr>
  		  <td align="center" class="titulos"><strong>Administrador la Comunidad Ibero Opina</strong></td>
        ...
        <tr>
          <td colspan="2" align="center" class="titulo_tablas"> Opiniones <?php echo ($startRow_opinions + 1) ?> - <?php echo min($startRow_opinions + $maxRows_opinions, $totalRows_opinions) ?> de <?php echo $totalRows_opinions ?></td>
          <td width="65" align="center"><input name="agrega" type="button" value="&nbsp;" title="Agregar Nuevo" style="background-color:transparent; border:0; background-image:url(../images/buttons/agregar.gif); background-repeat:no-repeat; width:25px; height:25px; cursor:pointer; outline:none;" onclick="window.location='insert_opinion.php'"/></td>
        </tr>
        <?php do { ?>
          <tr>
    <td width="83" height="70" align="center" valign="middle"><img src="../uploads/images/community_opinions/thumbnails/<?php echo $row_opinions['thumbnail'];?>" width="60" height="60" /></td>
    <td width="510" class="contenido"><?php echo $row_opinions['full_name'];?><br />
      <?php echo $row_opinions['job_position'];?></td>
    <td align="center"><input name="edita" type="button" value="&nbsp;" style="background-color:transparent; border:0; background-image:url(../images/buttons/editar.gif); background-repeat:no-repeat; width:25px; height:24px; cursor:pointer; outline:none" onclick="window.location='update_opinion.php?id_opinion=<?php echo $row_opinions['id_opinion'];?>'"/>
     &nbsp;
    <input name="elimina" type="button" value="&nbsp;" style="background-color:transparent; border:0; background-image:url(../images/buttons/eliminar.gif); background-repeat:no-repeat; width:20px; height:20px; cursor:pointer; outline:none"  onclick="window.location='delete_opinions.php?id_opinion=<?php echo $row_opinions['id_opinion'];?>'"/> 
      </td>
  </tr>
        ...
        <table border="0" align="center">
                ...
                <tr>
                  <td><?php if ($pageNum_opinions > 0) { // Show if not first page ?>
                      <a href="<?php printf("%s?pageNum_opinions=%d%s", $currentPage, 0, $queryString_opinions); ?>"><img src="../images/buttons/First.gif" border="0" /></a>
                      <?php } // Show if not first page ?></td>
                  <td><?php if ($pageNum_opinions > 0) { // Show if not first page ?>
                      <a href="<?php printf("%s?pageNum_opinions=%d%s", $currentPage, max(0, $pageNum_opinions - 1), $queryString_opinions); ?>"><img src="../images/buttons/Previous.gif" border="0" /></a>
                      <?php } // Show if not first page ?></td>
                  <td><?php if ($pageNum_opinions < $totalPages_opinions) { // Show if not last page ?>
                      <a href="<?php printf("%s?pageNum_opinions=%d%s", $currentPage, min($totalPages_opinions, $pageNum_opinions + 1), $queryString_opinions); ?>"><img src="../images/buttons/Next.gif" border="0" /></a>
                      <?php } // Show if not last page ?></td>
                  <td><?php if ($pageNum_opinions < $totalPages_opinions) { // Show if not last page ?>
                      <a href="<?php printf("%s?pageNum_opinions=%d%s", $currentPage, $totalPages_opinions, $queryString_opinions); ?>"><img src="../images/buttons/Last.gif" border="0" /></a>
                      <?php } // Show if not last page ?></td>
                </tr>
        </table>
