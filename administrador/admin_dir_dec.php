...
$query_programas = "SELECT * FROM site_directory ORDER BY id_encargado DESC";
...
function eliminar_prog(id_programa){
  ...
  if(r == true){
    window.location="delete_encargado.php?id_encargado="+id_programa;
  }if(r == false){
    //
  }
}
...
<table  border="0" cellpadding="5" cellspacing="0" class="tablas">
	<tr class="titulo_tabla">
		...
		<td>Correo</td>
		<td colspan="2">
        <input type="button" value="Nuevo Encargado" onclick="javascript:window.location='nuevo_dir_dec.php'">
        </td>
	  </tr>
	<?php $cont = 0; do { $cont++; ?>
		<tr>
			<td><?php echo $row_programas['nombre']; ?></a></td>
			<td><?php echo $row_programas['telefono']; ?></td>
			<td><?php echo $row_programas['extension']; ?></td>
			<td><?php echo $row_programas['correo']; ?></td>
			<td><a href="edit_encargado.php?id_encargado=<?php echo $row_programas['id_encargado']; ?>">Editar</a></td>
			<td><a onclick="eliminar_prog(<?php echo $row_programas['id_encargado']; ?>);" href="#">Eliminar</a></td>
		</tr>
